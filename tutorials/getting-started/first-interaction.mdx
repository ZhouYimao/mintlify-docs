...
<Info>
  **Key Insight**: After initiating with ⌘K, you don't need to wait on the current screen. You can think, start parallel work, or grab a coffee. When your input is needed, you'll get a notification and use ⌘J to jump directly to your Inbox.
</Info>

**The Flow:**
1. **⌘K** → Launch your request ("create marketing plan for Q2")
2. **Continue your day** → Think about other priorities, start new tasks, take breaks
3. **Get notified** → When agents have questions or results ready
4. **⌘J** → Jump to Inbox to review, provide feedback, or approve
5. **Repeat** → Keep multiple conversations and tasks flowing in parallel

**Example Parallel Workflow:**
```
9:00 AM: ⌘K → "Analyze last month's sales data"
9:01 AM: ⌘K → "Draft email to investors about product update"
9:02 AM: ⌘K → "Schedule team meeting for Friday"

[Go grab coffee, attend other meetings, think strategically]

10:30 AM: Get notification → Sales analysis ready for review
10:30 AM: ⌘J → Review analysis, ask follow-up questions
10:35 AM: Get notification → Investor email draft ready
10:35 AM: ⌘J → Approve email with minor edits
```
...
<Steps>
  ...
  <Step title="The Complete Cycle">
    ...
    2. **Continue other work** → No need to wait on screen
    3. **Get notified** → When team responds or agents have updates
    4. **⌘J** → Jump to Inbox to see responses
    5. Reply directly from Inbox or jump to full conversation
  </Step>
</Steps>
...
**Throughout the day**: Use ⌘K to initiate new work (then move on!)
```
⌘K → "create issue for bug fix" [go to other tasks]
⌘K → "schedule team meeting" [grab coffee]
⌘K → "ask @research-agent about metrics" [attend meeting]

# With voice input (after pressing ⌘K):
Speak: "Ask dev agent to review the login code" [continue coding]
Speak: "Create meeting with Alice and Bob about the new feature" [check emails]
```
